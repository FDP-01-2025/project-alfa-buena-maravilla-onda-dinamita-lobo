...
int main()
{
    ...
    { // do
        ...
        if (cin.fail())
        {                // Si la entrada no fue un número
        ...
        while (option < 1 || option > 3)
        {
            ...
            cout << "=========================================\n";
            cout << " 1. Jugar\n 2. Tabla de puntuaciones\n 3. Salir\n";
            cout << "Opcion inválida. Ingrese una opcion correcta: ";
            cin >> option;
        ...
        {
        case 1:
        { // Se utiliza un bloque para la declaración de variables dentro del case
            cout << "\n";
            ...
            while (gameActive)
            {
                ...
                do
                {
                    candidatePlayer = (candidatePlayer + 1) % numPlayers; // Avanza al siguiente jugador
                    rotationAttempts++;
                ...
                if (!gameActive)
                {          // Si el juego se marcó como inactivo dentro del bucle de búsqueda
                ...
                char accuseResponse; //var desubicada
                int accuserIndex = -1; //var desubicada
                int accusedIndex = currentTurn; // El jugador actual es el acusado por defecto //var desubicada

                // Preguntar si alguien quiere acusar
                ...
                {
                    cout << "Respuesta incorrecta. Ingresa (S) o (N): ";
                    cin >> accuseResponse;
                    cin.ignore(numeric_limits<streamsize>::max(), '\n');
                }
                ...
                {
                    bool seleccionValida = false; //var desubicada
                    while (!seleccionValida)
                    {
                        // Mostrar siempre la lista de jugadores primero
                        cout << "=== JUGADORES DISPONIBLES PARA ACUSAR ===\n";
                        for (int i = 0; i < numPlayers; i++)
                        {
                            if (i != currentTurn)
                            { // No se puede acusar a sí mismo
                                cout << "" << i + 1 << "." << playerNames[i] << "\n";
                            }
                        }
                        cout << "========================================\n";

                        // Obtener entrada del acusador
                        cout << "Ingrese el NUMERO del jugador que acusa: ";
                        if (cin >> accuserIndex)
                        {
                            accuserIndex--; // Ajustar a índice base 0

                            // Validar el rango y que no sea auto-acusación
                            if (accuserIndex >= 0 && accuserIndex < numPlayers && accuserIndex != currentTurn)
                            {
                            ...
                            else
                            {
                                ...
                                {
                                    cout << "¡ERROR! No puedes acusarte a ti mismo.\n\n";
                                }
                                else
                                {
                                    cout << "¡ERROR! Numero de jugador invalido.\n\n";
                                }
                            }
                        }
                        else
